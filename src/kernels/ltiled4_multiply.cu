...
__device__ __forceinline__ void load_tile_fast(const T* __restrict__ mat,
                                               ...
                                               T* __restrict__ smat) {
  // Load sMatRow * sMatCol elements to sMat with THREAD_PER_BLOCK threads
  // caller owns the commit so one group covers both tiles
  constexpr int iter_count = (sMatRow * sMatCol) / (THREAD_PER_BLOCK * 4);

  // cp.async takes 4/8/16B and needs both ends aligned to it; a padded sMatLd
  // can put the destination row off 16B, so take the widest granule that holds
  constexpr int kRowBytes = sMatLd * sizeof(T);
  constexpr int kThreadBytes = 4 * sizeof(T);
  constexpr int kGran = (kRowBytes % 16 == 0 && kThreadBytes % 16 == 0) ? 16
                        : (kRowBytes % 8 == 0 && kThreadBytes % 8 == 0) ? 8
                                                                        : 4;
  constexpr int kElemPerChunk = kGran / sizeof(T);
  constexpr int kChunks = 4 / kElemPerChunk;
  static_assert(kElemPerChunk >= 1 && 4 % kElemPerChunk == 0);
...
#pragma unroll
    for (int c = 0; c < kChunks; ++c) {
      auto e = col + c * kElemPerChunk;
      __pipeline_memcpy_async(&smat[ID2X(row, e, sMatLd)],
                              &mat[ID2X(row, e, mat_ld)], kGran);
    }
  }
};
...
__device__ __forceinline__ void load_tile(const T* __restrict__ mat, int mat_ld,
  ...
  if (fast_predicate) {
    load_tile_fast<T, sMatLd, sMatRow, sMatCol, THREAD_PER_BLOCK>(mat, mat_ld,
                                                                  smat);
  } else {
...
__global__ void __launch_bounds__(THREAD_PER_BLOCK, 1)
  ...
  bool p_fast_A_load = (m % bM == 0) && (k % bK == 0);
  bool p_fast_B_load = (n % bN == 0) && (k % bK == 0);
